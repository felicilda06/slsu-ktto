...
if ($apiType) {
   if ($apiType === 'add_new_study') {
      ...
      if (isset($_FILES['updt_file'])) {
         ...
         $query = "Update tbl_studies set title = '" . $_POST['updt_title'] . "', proponent= '" . $_POST['updt_proponent'] . "', technology_type = '" . $_POST['updt_technology_type'] . "', contact_information = '" . $_POST['updt_contact_information'] . "', intellectual_property = '".$_POST['updt_intellectual_property']."', college = '".$_POST['updt_college']."' where id = '" . $_POST['rowId'] . "'";
         $executeQuery = mysqli_query($conn, $query);
         if ($executeQuery) {
            $response->status_code = 200;
            $response->message = "Study Successfully Updated.";
      ...
      echo $_POST['studyId'];
   } else if ($apiType === 'fetch_college') {
      $query = "Select * from tbl_college where userId = '" . $_POST['userId'] . "' or userId = 0";
      $executeQuery = mysqli_query($conn, $query);
      $arr_college = array();
      while ($row = mysqli_fetch_assoc($executeQuery)) {
         $arr_college[] = $row;
      ...
      echo json_encode($arr_college);
   } else if ($apiType === 'create_new_college') {
      $query = "Insert into tbl_college values ('', '" . $_POST['collegeName'] . "', '" . $_POST['userId'] . "', 0)";
      $executeQuery = mysqli_query($conn, $query);
      $response = new Response();
      if ($executeQuery) {
         $response->status_code = 200;
         $response->message = "New College Successfully Added.";
      } else {
         $response->status_code = 400;
         $response->message = "Something went wrong on adding item.Please try again.";
      }
      echo json_encode($response);
   }
} else {
