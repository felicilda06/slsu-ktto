    ...
    $('#tbl_body_drafter_log  tr.fetch_logs').remove();
     if(logs.length > 0){
        logs.map((log, i)=> {
            ...
            $('#tbl_body_drafter_log').append(`<tr key="${i}" id="${log?.id}" class='fetch_logs ${userType == 'patent drafter' ? log?.id === activeRow ? 'selected': '': ''}' style="background: ${color}; ${userType !== "patent drafter" ? "pointer-events: none;": "pointer-events: fill;"}">
